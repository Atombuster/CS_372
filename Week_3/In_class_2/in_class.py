def packet_complete(data):
    ...
    if length < 2:
        return False
    string_length = int.from_bytes(data[0:2], "big")
    
    if string_length + 2 <= length:
        return True
    return False
...
def packet_extract(data):
    ...
    if length < 2:
        return None
    string_length = int.from_bytes(data[0:2], "big")
    bytes_return = data[0: string_length+2]
    bytes_left = data[string_length+2:]
    return (bytes_return, bytes_left)
