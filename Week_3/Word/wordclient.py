...
def get_next_word_packet(s):
    ...
    total_length = WORD_LEN_SIZE + word_length

    while len(packet_buffer) < total_length:
        data = s.recv(1024)
        if not data:
            return None
        packet_buffer += data

    word_packet = packet_buffer[:total_length]
    packet_buffer = packet_buffer[total_length:]
    return word_packet
...
def extract_word(word_packet):
    ...
    length = len(word_packet)
    if length < 2:
        return None
    string_length = int.from_bytes(word_packet[0:2], "big")
    bytes_return = word_packet[0: string_length+2]
    return (bytes_return.decode())
